...
HOST="$1"
BASE_OID="1.3.6.1.4.1.34774.4.1.23.5.4.1.2"  # 修改为 FAN 的 OID
CACHE="/tmp/fan_oid_cache.txt"
...
while IFS= read -r line; do
    fan_name=$(echo "$line" | awk -F ' = STRING: ' '{print $2}' | tr -d '"' | tr -d '\r')
    if [ -n "$fan_name" ]; then
        if $first; then
            first=false
        else
